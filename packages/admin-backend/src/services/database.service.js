...
const getTenantConnection = async (dbName) => {
  ...
  if (tenantConnections[connectionKey]) {
    ...
    debugLog(2, `Cached connection stale (state: ${conn.readyState}). Reconnecting: ${connectionKey}`)
  ...
  const events = ["connecting", "connected", "open", "disconnecting", "disconnected", "close", "reconnected", "error", "fullsetup"]

  events.forEach((event) => {
    connection.on(event, (arg) => {
      debugLog(3, `📡 [${connectionKey}] Event: ${event}`, arg ? util.inspect(arg, { depth: 1 }) : "")
...
const getTenantModels = (connection) => {
  ...
  const models = {
    dbConnection: connection, // Store connection here
  }
