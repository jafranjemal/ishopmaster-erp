...
class PaymentsService {
  ...
  async recordPayment(
    ...
    try {
      ...
      const { Supplier, Customer, Payment, PaymentMethod, Cheque, SupplierInvoice, SalesInvoice, PrintJob } = this._initModels(
        models,
        session
      )
      ...
      const hasCashPayment = paymentLines.some((line) => {
        const method = methodMap.get(line.paymentMethodId.toString())
        return method?.type === "cash"
      })
      if (hasCashPayment) {
        try {
          // Find the default cash drawer for the branch where the sale occurred.
          const cashDrawer = await models.HardwareDevice.findOne({
          ...
          if (cashDrawer) {
            // Create a job for the Hardware Bridge to open the drawer.
            // Note: We use the PrintJob schema for simplicity, but a generic HardwareJob schema would be ideal.
            await PrintJob.create(
              [
                {
                  documentType: "CASH_DRAWER_OPEN",
                  documentId: sourceDocument._id, // Link to the invoice for auditing
                  hardwareDeviceId: cashDrawer._id,
                  status: "pending",
                  copies: 1,
                  branchId: sourceDocument.branchId,
                  createdBy: userId,
            ...
            console.warn(
              `[PaymentsService] Cash payment received, but no default cash drawer is configured for branch ${sourceDocument.branchId}.`
            )
          }
        } catch (error) {
          // Log the error but do not fail the entire payment transaction.
          console.error(`[PaymentsService] Failed to create cash drawer job for invoice ${sourceDocument._id}:`, error)
        }
      }
