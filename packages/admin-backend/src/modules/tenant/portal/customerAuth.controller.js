...
exports.requestFullLoginLink = asyncHandler(async (req, res, next) => {
  ...
  const initialToken = await CustomerAuthToken.findOne({ token, status: "used" }).populate("customerId")
  ...
  res.status(200).json({ success: true, message: `Login link sent to ${initialToken.customerId.email}` })
})
