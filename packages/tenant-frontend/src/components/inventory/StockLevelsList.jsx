    ...
    <Table>
      ...
      <TableBody>
        {stockLevels.map((stock, index) => (
          ...
          <TableRow key={`${stock.ProductVariantsId}-${stock.branchId}-${index}`}>
            <TableCell className="font-medium">
              <Link to={`/inventory/stock-details/${stock.ProductVariantsId}`} className="hover:text-indigo-300 hover:underline">
                {stock.variantName}
              </Link>
            </TableCell>
